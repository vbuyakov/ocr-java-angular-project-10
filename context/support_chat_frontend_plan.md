...
overview: Feature-driven Angular 21 app for CU/AU support chat aligned with [ws-protocol.md](ws-protocol.md), REST/STOMP from [support_chat_api_endpoints.md](support_chat_api_endpoints.md). TailwindCSS 4 for Bootstrap-like UI, strict TypeScript (no `any`), signals-first. Brand logo in [../web/public/brand/ycyw-logo.png](../web/public/brand/ycyw-logo.png).
todos:
...
**Normative flow:** [ws-protocol.md](ws-protocol.md)  
**REST & STOMP reference:** [support_chat_api_endpoints.md](support_chat_api_endpoints.md) (and full spec if needed)

**Backend** implementation lives under `api/`; this plan covers **`web/`** only.

**Plan copies:** [context/support_chat_frontend_plan.md](support_chat_frontend_plan.md) (this file) and [`.cursor/plans/support_chat_frontend.plan.md`](../.cursor/plans/support_chat_frontend.plan.md) (Cursor checklist). Keep them in sync when editing.
...
| **Structure** | **Feature-driven** top-level areas under `app/`: **`core`** (platform), **`auth`** (identity), **`tchat`** (support chat — CU + AU); facades under `tchat/customer` and `tchat/agent` |
...
| **Playwright** | **Phase 6 — regression:** expand specs with **projects** or loops for **viewport presets** (mobile / tablet / desktop); run full **smoke** from [ws-protocol.md](ws-protocol.md) (CU + AU); add **`test:e2e:headed`** to package scripts if not already. Prefer **`--ui`** for exploratory passes. |
